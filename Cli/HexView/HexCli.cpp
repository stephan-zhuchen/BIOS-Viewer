...
void HexCliView::calculateLayout()
{
    ...
    // 地址区起始X: 1 (留出边框或空白)
    // 十六进制数据区起始X: addressWidth + 1 (分隔符) + 1 (空格) = addressWidth + 2 (如果地址后有空格，则是 addressWidth + 1 + 1)
    // 我们在drawHexContent中具体定位
...
void HexCliView::drawHexContent()
{
         ...
         ++line_idx, ++draw_y_in_win)
    {
        ...
        addrStrStream << "0x" << std::hex << std::setw(addressWidth - 2 - 1)
                      << std::setfill('0') << currentFileOffset;
        mvwprintw(hexWin, DATA_START_ROW_IN_WINDOW + draw_y_in_win, addr_text_print_x, "%s", addrStrStream.str().c_str());
        ...
        int validBytesInLine = std::min(static_cast<INT64>(BYTES_PER_LINE),
                                        currentBinaryData->InputImageSize - currentFileOffset);
        ...
        for (int byte_col_idx = 0; byte_col_idx < BYTES_PER_LINE; ++byte_col_idx)
        {
            if (byte_col_idx < validBytesInLine)
            {
                ...
                bool isCursorOnThisByte = (currentMode == Mode::EDIT &&
                                           line_idx == editCursorLine &&
                ...
                mvwaddch(hexWin, DATA_START_ROW_IN_WINDOW + draw_y_in_win, ascii_data_print_x + byte_col_idx, displayChar);
            }
            else
            {                                                                                                            // 如果行不满，用空格填充剩余部分
                mvwprintw(hexWin, DATA_START_ROW_IN_WINDOW + draw_y_in_win, hex_data_print_x + byte_col_idx * 3, "   "); // 3 spaces for "XX "
...
void HexCliView::drawStatusBar()
{
    ...
    else
    {
        ...
        int totalLinesDisplay = (currentBinaryData->InputImageSize > 0) ? (static_cast<int>((currentBinaryData->InputImageSize + BYTES_PER_LINE - 1) / BYTES_PER_LINE)) : 0;
        ...
        rightStatus << "Offset:0x" << std::hex << std::setw(addressWidth - 3) << std::setfill('0') << currentByteOffsetDisplay
                    << "  line:" << std::dec << currentLineDisplay << "/" << totalLinesDisplay
                    << "  size:" << currentBinaryData->InputImageSize;
...
void HexCliView::moveEditCursor(int dLine, int dByteOrNibble, bool isNibbleMove)
{
    ...
    {                                      // 按半字节移动 (通常是输入十六进制字符后)
        ...
        if (editCursorNibble > 1)
        { // 从低半位前进到高半位 (下一字节)
            editCursorNibble = 0;
            editCursorByteInLine++;
        }
        else if (editCursorNibble < 0)
        { // 从高半位后退到低半位 (前一字节)
            editCursorNibble = 1;
            editCursorByteInLine--;
        }
    ...
    const int totalDataLines = (currentBinaryData->InputImageSize > 0) ? (static_cast<int>((currentBinaryData->InputImageSize + BYTES_PER_LINE - 1) / BYTES_PER_LINE)) : 1; // 至少为1行（即使是空文件，逻辑上也有第0行）
    ...
    if (editCursorLine == totalDataLines - 1 && currentBinaryData->InputImageSize > 0)
    {
        ...
        else if (editCursorByteInLine >= bytesInLastLine)
        { // 如果不小心超出了最后一个字节
            editCursorByteInLine = std::max(0, bytesInLastLine - 1);
            editCursorNibble = 0; // 或1，取决于你想要的行为
        }
    }
    else if (currentBinaryData->InputImageSize == 0)
    { // 空文件特殊处理
        editCursorLine = 0;
        editCursorByteInLine = 0;
        editCursorNibble = 0;
...
void HexCliView::handleNormalModeInput(int ch)
{
    ...
    int totalDataLines = (currentBinaryData->InputImageSize > 0) ? (static_cast<int>((currentBinaryData->InputImageSize + BYTES_PER_LINE - 1) / BYTES_PER_LINE)) : 0;
...
void HexCliView::handleEditModeInput(int ch)
{
    ...
    default:
    {
        ...
        if (offset < currentBinaryData->InputImageSize)
        { // 确保在文件范围内
            UINT8 currentByte = currentBinaryData->InputImage[offset];
            if (editCursorNibble == 0)
            { // 修改高半位
...
void HexCliView::handleCommandLineInput(int ch)
{
    ...
    case '\r':
        ...
        // 否则，通常会回到NORMAL模式
        if (currentMode == Mode::COMMAND_LINE && commandString.find("Error:") == std::string::npos && commandString.find("Unsaved") == std::string::npos)
        {
    ...
    case 8:   // ASCII backspace
        ...
        break;
    default:
        if (isprint(ch) && commandString.length() < (size_t)totalWidth - 2 - 5)
        { // -2 for ':', -5 for some margin
    ...
    while (running)
    {
        ...
        if (currentMode != Mode::COMMAND_LINE)
        {
            if (!last_status_message.empty() &&
                last_status_message.find("Error:") == std::string::npos &&
        ...
        if (currentMode == Mode::NORMAL)
        {
            ...
            if (ch == 'q' && currentMode == Mode::NORMAL)
            { // 在普通模式下按 'q'
                if (!dataModified)
                {
                    running = false; // 直接退出
