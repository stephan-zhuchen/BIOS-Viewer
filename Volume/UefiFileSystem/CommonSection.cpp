...
Depex::Depex(UINT8 *file, INT64 length) : data(file), size(length) {
    INT64 offset = 0;
    ...
    while (Opcode != EFI_DEP_END) {
        ...
        } else if (Opcode == EFI_DEP_BEFORE || Opcode == EFI_DEP_AFTER || Opcode == EFI_DEP_TRUE
                   || Opcode == EFI_DEP_FALSE) {
...
void CommonSection::DecodeChildVolume() {
    ...
    case EFI_SECTION_COMPRESSION:
        ...
        if (CompressionType == EFI_STANDARD_COMPRESSION) {
            ScratchSize = 0;
            status =
                UefiDecompressGetInfo(data + HeaderSize, (UINT32) size - HeaderSize, &decompressedSize, &ScratchSize);
    ...
    case EFI_SECTION_GUID_DEFINED:
        ...
        if (GuidDefinedSection.SectionDefinitionGuid == GuidDatabase::gEfiCertTypeRsa2048Sha256Guid) {
            ...
            while (offset < size) {
                auto ChildSection =
                    new CommonSection(data + offset, HeaderSize, offsetFromBegin + offset, Compressed, this);
        ...
        // Lzma Decompress
            ...
            status = LzmaUefiDecompressGetInfo(
                data + HeaderSize, (UINT32) size - HeaderSize, &decompressedSize, &ScratchSize);
            if (status != RETURN_SUCCESS) {
        ...
        // Brotli Decompress
            ...
            ScratchSize = 0;
            status = BrotliUefiDecompressGetInfo(
                data + HeaderSize, (UINT32) size - HeaderSize, &decompressedSize, &ScratchSize);
            if (status != RETURN_SUCCESS) {
    ...
    case EFI_SECTION_USER_INTERFACE:
        ...
        if (FileNameString == "FmpDxe") {
            ParentVolume->setUniqueVolumeName(FileNameString + " "
                                              + GuidDatabase::getFmpDeviceName(ParentVolume->getVolumeGuid()));
        } else {
            ParentVolume->setUniqueVolumeName(FileNameString);
        }
    ...
    case EFI_SECTION_RAW:
        if (ParentVolume->getVolumeSubType() == VolumeType::Apriori) {
            ...
            INT64 RemainingSize = size - (INT64) HeaderSize;
            while (RemainingSize >= (INT64) sizeof(EFI_GUID)) {
...
Volume *CommonSection::Reorganize() {
    ...
    if (AcpiClass::isAcpiHeader(data + HeaderSize, size - HeaderSize)) {
        AcpiClass *acpiVolume =
            new AcpiClass(data + HeaderSize, size - HeaderSize, offsetFromBegin + HeaderSize, false);
