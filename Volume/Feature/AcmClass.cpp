...
void AcmHeaderClass::setInfoStr() {
    ...
    ss << setw(width) << "Guid:" << AcmInfoTable->Guid.str(true) << "\n"
       << setw(width) << "Acm Version:" << (UINT32) AcmVersion.AcmMajorVersion << "."
       << (UINT32) AcmVersion.AcmMinorVersion << "." << (UINT32) AcmVersion.AcmRevision << "\n";
    ...
    ss << setw(width) << "ModuleType:" << hex << uppercase << acmHeader.ModuleType << "h\n"
       ...
       << setw(width) << "Date:" << hex << uppercase << (acmHeader.Date >> 16) << "-"
       << ((acmHeader.Date & 0xFF00) >> 8) << "-" << (acmHeader.Date & 0xFF) << "\n"
