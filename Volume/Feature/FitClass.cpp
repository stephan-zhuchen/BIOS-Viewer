...
INT64 FitTableClass::SelfDecode() {
    ...
    if (FitSignature == (UINT64) FIT_SIGNATURE) {
        ...
        UINT8 Checksum =
            CalculateSum8((UINT8 *) (data + FitTableAddress), sizeof(FIRMWARE_INTERFACE_TABLE_ENTRY) * FitEntryNum);
        if (Checksum == 0) {
        ...
        for (INT64 index = 1; index < FitEntryNum; ++index) {
            FIRMWARE_INTERFACE_TABLE_ENTRY FitEntry =
                *(FIRMWARE_INTERFACE_TABLE_ENTRY *) (data + FitTableAddress
                                                     + sizeof(FIRMWARE_INTERFACE_TABLE_ENTRY) * index);
            ...
            if (FitEntry.Type == FIT_TABLE_TYPE_MICROCODE) {
                ...
                if (AcmEntry->isValid()) {
                    ...
                    delete AcmEntry;
                       ...
                       || FitEntry.Type == FIT_TABLE_TYPE_BOOT_POLICY_MANIFEST) {
