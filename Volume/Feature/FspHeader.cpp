...
FspHeader::FspHeader(UINT8 *buffer, INT64 length, INT64 offset) : Volume(buffer, length, offset, false, nullptr) {
}
...
void FspHeader::setInfoStr() {
    ...
    ss << setw(width) << "SpecVersion:" << hex << uppercase << (UINT16) mTable.FspInfoHeader.SpecVersion << "h\n"
       ...
       << setw(width) << "TempRamInitEntryOffset:" << hex << uppercase << mTable.FspInfoHeader.TempRamInitEntryOffset
       << "h\n"
       << setw(width) << "NotifyPhaseEntryOffset:" << hex << uppercase << mTable.FspInfoHeader.NotifyPhaseEntryOffset
       << "h\n"
       << setw(width) << "FspMemoryInitEntryOffset:" << hex << uppercase
       << mTable.FspInfoHeader.FspMemoryInitEntryOffset << "h\n"
       << setw(width) << "TempRamExitEntryOffset:" << hex << uppercase << mTable.FspInfoHeader.TempRamExitEntryOffset
       << "h\n"
       << setw(width) << "FspSiliconInitEntryOffset:" << hex << uppercase
       << mTable.FspInfoHeader.FspSiliconInitEntryOffset << "h\n"
       << setw(width) << "FspMultiPhaseSiInitEntryOffset:" << hex << uppercase
       << mTable.FspInfoHeader.FspMultiPhaseSiInitEntryOffset << "h\n"
       << setw(width) << "ExtendedImageRevision:" << hex << uppercase << mTable.FspInfoHeader.ExtendedImageRevision
       << "h\n"
       << setw(width) << "FspMultiPhaseMemInitEntryOffset:" << hex << uppercase
       << mTable.FspInfoHeader.FspMultiPhaseMemInitEntryOffset << "h\n"
       << setw(width) << "FspSmmInitEntryOffset:" << hex << uppercase << mTable.FspInfoHeader.FspSmmInitEntryOffset
       << "h\n"
       << setw(width) << "FspProducerId:" << hex << uppercase
       << charToString(mTable.FspInfoExtendedHeader.FspProducerId, 6) << "\n"
       << setw(width) << "FspProducerRevision:" << hex << uppercase << mTable.FspInfoExtendedHeader.FspProducerRevision
       << "h\n"
       << setw(width) << "FspProducerDataSize:" << hex << uppercase << mTable.FspInfoExtendedHeader.FspProducerDataSize
       << "h\n"
