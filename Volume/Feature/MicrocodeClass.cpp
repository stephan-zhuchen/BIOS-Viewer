...
INT64 MicrocodeHeaderClass::SelfDecode() {
    ...
    UINT32 ExtendedTableLength =
        microcodeHeader.TotalSize - (microcodeHeader.DataSize + (UINT32) sizeof(CPU_MICROCODE_HEADER));
    if (ExtendedTableLength != 0) {
        ExtendedTableHeader =
            (CPU_MICROCODE_EXTENDED_TABLE_HEADER *) (data + microcodeHeader.DataSize + sizeof(CPU_MICROCODE_HEADER));
        if ((ExtendedTableLength > sizeof(CPU_MICROCODE_EXTENDED_TABLE_HEADER)) && ((ExtendedTableLength & 0x3) == 0)) {
            ...
            if (ExtendedTableCount <= (ExtendedTableLength - sizeof(CPU_MICROCODE_EXTENDED_TABLE_HEADER))
...
void MicrocodeHeaderClass::setInfoStr() {
    ...
    if (!isEmpty && !isCorrupted) {
        ss << std::setw(width) << "HeaderVersion:" << std::hex << std::uppercase << microcodeHeader.HeaderVersion
           << "h\n"
           << std::setw(width) << "UpdateRevision:" << std::hex << std::uppercase << microcodeHeader.UpdateRevision
           << "h\n"
           << std::setw(width) << "Date:" << std::hex << std::uppercase << microcodeHeader.Date.Bits.Year << "-"
           << microcodeHeader.Date.Bits.Month << "-" << microcodeHeader.Date.Bits.Day << "\n"
           << std::setw(width) << "ProcessorSignature:" << std::hex << std::uppercase
           << microcodeHeader.ProcessorSignature.Uint32 << "h\n"
           << std::setw(width) << "Checksum:" << std::hex << std::uppercase << microcodeHeader.Checksum << "h\n"
           << std::setw(width) << "LoaderRevision:" << std::hex << std::uppercase << microcodeHeader.LoaderRevision
           << "h\n"
           << std::setw(width) << "ProcessorFlags:" << std::hex << std::uppercase << microcodeHeader.ProcessorFlags
           << "h\n"
    ...
    if (!ExtendedMicrocodeList.empty()) {
        for (auto ExtendedMicrocode : ExtendedMicrocodeList) {
            ss << "\nExtended Microcode Info:" << "\n"
               << std::setw(width) << "ProcessorSignature:" << std::hex << std::uppercase
               << ExtendedMicrocode.ProcessorSignature.Uint32 << "h\n";
        }
    }
