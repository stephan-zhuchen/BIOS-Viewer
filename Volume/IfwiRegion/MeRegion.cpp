...
MeRegion::MeRegion(UINT8 *buffer, INT64 length, INT64 offset) : Volume(buffer, length, offset, false, nullptr) {
}
...
INT64 MeRegion::SelfDecode() {
    ...
    while (SearchOffset < size - (INT64) sizeof(INT64)) {
        if (*(UINT32 *) (data + SearchOffset) == ME_VERSION_SIGNATURE
            || *(UINT32 *) (data + SearchOffset) == ME_VERSION_SIGNATURE2) {
            VersionFound = true;
            break;
        }
        SearchOffset += 4;
...
void MeRegion::setInfoStr() {
    ...
    stringstream ss;
    ss.setf(ios::left);
    ss << setw(width) << "Version Signature:" << hex
       << BaseLibrarySpace::charToString((CHAR8 *) &MeVersion.Signature, sizeof(UINT32), false) << "\n"
       << setw(width) << "ME Version:" << dec << MeVersion.Major << "." << MeVersion.Minor << "." << MeVersion.Bugfix
       << "." << MeVersion.Build << "\n";
    InfoStr = ss.str();
}
...
void CSE_LayoutClass::DecodeChildVolume() {
    if (Ver == IFWI_Ver::IFWI_17) {
        ...
        for (int i = 0; i < BOOT_PARTITION_NUM; ++i) {
            ...
            auto *BootPartition =
                new CSE_PartitionClass(data + ifwiHeader.ifwi17Header.BootPartition[i].Offset,
        ...
        std::sort(CSE_Partitions.begin(), CSE_Partitions.end(), [](CSE_PartitionClass *p1, CSE_PartitionClass *p2) {
...
void CSE_LayoutClass::setInfoStr() {
    ...
    case IFWI_Ver::IFWI_16:
        ...
        width = 25;
        ss << setw(width) << "Data partition offset:" << hex << uppercase
           << ifwiHeader.ifwi16Header.DataPartition.Offset << "h\n"
           << setw(width) << "Data partition size:" << hex << uppercase << ifwiHeader.ifwi16Header.DataPartition.Size
           << "h\n";
        for (int i = 0; i < BOOT_PARTITION_NUM; ++i) {
            ss << setw(width) << "Data partition offset:" << hex << uppercase
               << ifwiHeader.ifwi16Header.BootPartition[i].Offset << "h\n"
               << setw(width) << "Data partition size:" << hex << uppercase
               << ifwiHeader.ifwi16Header.BootPartition[i].Size << "h\n";
        }
        break;
    case IFWI_Ver::IFWI_17:
        ...
        width = 25;
        ss << setw(width) << "Data partition offset:" << hex << uppercase
           << ifwiHeader.ifwi17Header.DataPartition.Offset << "h\n"
           << setw(width) << "Data partition size:" << hex << uppercase << ifwiHeader.ifwi17Header.DataPartition.Size
           << "h\n";
        for (int i = 0; i < BOOT_PARTITION_NUM; ++i) {
            ss << setw(width) << "Data partition offset:" << hex << uppercase
               << ifwiHeader.ifwi17Header.BootPartition[i].Offset << "h\n"
               << setw(width) << "Data partition size:" << hex << uppercase
               << ifwiHeader.ifwi17Header.BootPartition[i].Size << "h\n";
        }
        break;
    default:
        break;
...
CSE_PartitionClass::CSE_PartitionClass(
    UINT8 *file, INT64 RegionLength, INT64 offset, Volume *parent, string name, PartitionLevel lv)
...
void CSE_PartitionClass::decodeBootPartition() {
    ...
    for (UINT16 i = 0; i < numEntries; i++) {
        ...
        if (ptEntry->Size == 0)
            continue;
        auto ChildPartition = new CSE_PartitionClass(data + ptEntry->Offset,
                                                     ptEntry->Size,
                                                     offsetFromBegin + ptEntry->Offset,
                                                     this,
                                                     name,
                                                     PartitionLevel::Level2);
        ChildPartition->SelfDecode();
        ChildVolume.push_back(ChildPartition);
    }
    std::sort(ChildVolume.begin(), ChildVolume.end(), [](Volume *p1, Volume *p2) {
        return p1->getOffset() < p2->getOffset();
    });
}
...
void CSE_PartitionClass::decodeDataPartition() {
    ...
    for (UINT16 i = 0; i < numEntries; i++) {
        ...
        if (fptEntry->Size == 0)
            continue;
        auto ChildPartition = new CSE_PartitionClass(data + fptEntry->Offset,
                                                     fptEntry->Size,
                                                     offsetFromBegin + fptEntry->Offset,
                                                     this,
                                                     name,
                                                     PartitionLevel::Level2);
        ChildPartition->SelfDecode();
...
string CSE_PartitionClass::bpdtEntryTypeToString(UINT16 type) {
    switch (type) {
    case BPDT_ENTRY_TYPE_SMIP:
        return "OEM SMIP";
    case BPDT_ENTRY_TYPE_RBEP:
        return "CSE RBE Partition";
    case BPDT_ENTRY_TYPE_FTPR:
        return "Bring Up";
    case BPDT_ENTRY_TYPE_UCOD:
        return "Microcode";
    case BPDT_ENTRY_TYPE_IBBP:
        return "IBB";
    case BPDT_ENTRY_TYPE_S_BPDT:
        return "Secondary BPDT";
    case BPDT_ENTRY_TYPE_OBBP:
        return "OBB";
    case BPDT_ENTRY_TYPE_NFTP:
        return "CSE Main Partition";
    case BPDT_ENTRY_TYPE_ISHC:
        return "ISH Partition";
    case BPDT_ENTRY_TYPE_DLMP:
        return "Debug Launch Module";
    case BPDT_ENTRY_TYPE_UEBP:
        return "IFP Bypass";
    case BPDT_ENTRY_TYPE_UTOK:
        return "Debug Tokens";
    case BPDT_ENTRY_TYPE_UFS_PHY:
        return "UFS PHY Config";
    case BPDT_ENTRY_TYPE_UFS_GPP_LUN:
        return "UFS GPP LUN";
    case BPDT_ENTRY_TYPE_PMCP:
        return "PMC Partition";
    case BPDT_ENTRY_TYPE_IUNP:
        return "IUnit Partition";
    case BPDT_ENTRY_TYPE_NVMC:
        return "NVM Config";
    case BPDT_ENTRY_TYPE_UEP:
        return "Unified Emulation";
    case BPDT_ENTRY_TYPE_WCOD:
        return "CSE WCOD Partition";
    case BPDT_ENTRY_TYPE_LOCL:
        return "CSE LOCL Partition";
    case BPDT_ENTRY_TYPE_OEMP:
        return "OEM KM Partition";
    case BPDT_ENTRY_TYPE_FITC:
        return "fitc.cfg";
    case BPDT_ENTRY_TYPE_PAVP:
        return "PAVP";
    case BPDT_ENTRY_TYPE_IOMP:
        return "IOM Partition";
    case BPDT_ENTRY_TYPE_XPHY:
        return "NPHY Partition";
    case BPDT_ENTRY_TYPE_TBTP:
        return "TBT Partition";
    case BPDT_ENTRY_TYPE_PLTS:
        return "Platform Settings";
    case BPDT_ENTRY_TYPE_RES27:
        return "Reserved 27";
    case BPDT_ENTRY_TYPE_RES28:
        return "Reserved 28";
    case BPDT_ENTRY_TYPE_RES29:
        return "Reserved 29";
    case BPDT_ENTRY_TYPE_RES30:
        return "Reserved 30";
    case BPDT_ENTRY_TYPE_DPHY:
        return "SPHY Partition";
    case BPDT_ENTRY_TYPE_PCHC:
        return "SOCC Partition";
    case BPDT_ENTRY_TYPE_ISIF:
        return "ISI FW";
    case BPDT_ENTRY_TYPE_ISIC:
        return "ISI Config";
    case BPDT_ENTRY_TYPE_HBMI:
        return "HBM IO";
    case BPDT_ENTRY_TYPE_OMSM:
        return "OOB MSM";
    case BPDT_ENTRY_TYPE_GTGP:
        return "GT-GPU";
    case BPDT_ENTRY_TYPE_MDFI:
        return "MDF IO";
    case BPDT_ENTRY_TYPE_PUNP:
        return "PUnit";
    case BPDT_ENTRY_TYPE_PHYP:
        return "GSC PHY";
    case BPDT_ENTRY_TYPE_SAMF:
        return "SAM FW";
    case BPDT_ENTRY_TYPE_PPHY:
        return "PPHY";
    case BPDT_ENTRY_TYPE_GBST:
        return "GBST";
    case BPDT_ENTRY_TYPE_TCCP:
        return "TCC";
    case BPDT_ENTRY_TYPE_PSEP:
        return "PSE";
    case BPDT_ENTRY_TYPE_ESE:
        return "ESE Package";
    case BPDT_ENTRY_TYPE_ACE:
        return "ACE Package";
    case BPDT_ENTRY_TYPE_SPHY:
        return "SOC SPHY Partition";
    }
    return "Unknown";
}
