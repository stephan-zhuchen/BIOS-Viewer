...
GbeRegion::GbeRegion(UINT8 *buffer, INT64 length, INT64 offset) : Volume(buffer, length, offset, false, nullptr) {
}
...
void GbeRegion::setInfoStr() {
    ...
    ss << setw(width) << "MAC:" << hex << (UINT16) MacAddress.vendor[0] << ":" << (UINT16) MacAddress.vendor[1] << ":"
       << (UINT16) MacAddress.vendor[2] << ":" << (UINT16) MacAddress.device[0] << ":" << (UINT16) MacAddress.device[1]
