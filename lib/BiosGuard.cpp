...
namespace UefiSpace {
    ...
    BiosGuardClass::BiosGuardClass(UINT8* fv, INT64 address, INT64 length):Volume(fv, length, address) {
        BgupHeader = *(BGUP_HEADER*)fv;
        decodeBgsl(fv + sizeof(BGUP_HEADER), BgupHeader.ScriptSectionSize);
        INT64 BgupCSize = length - sizeof(BGUP_HEADER) - BgupHeader.ScriptSectionSize;
        BgupCHeader = *(BGUPC_HEADER*)(fv + sizeof(BGUP_HEADER) + BgupHeader.ScriptSectionSize);
        ...
        UINT8* Temp = fv + sizeof(BGUP_HEADER) + BgupHeader.ScriptSectionSize + sizeof(BGUPC_HEADER);
        ModulusData = new UINT8[ModulusSize];
        for (int idx = 0; idx < ModulusSize; ++idx) {
            ModulusData[idx] = Temp[ModulusSize - idx - 1];
        }

        UINT32 ModulusTail = *(UINT32*)(fv + sizeof(BGUP_HEADER) + BgupHeader.ScriptSectionSize + sizeof(BGUPC_HEADER) + ModulusSize);
        if (ModulusTail != 0x00010001) {
            cout << "invalid Algorithm" << endl;
        }

        UpdatePackageDigest = new UINT8[RSAKeySize];
        UINT8* Src = fv + sizeof(BGUP_HEADER) + BgupHeader.ScriptSectionSize + sizeof(BGUPC_HEADER) + ModulusSize + sizeof(ModulusTail);
    ...
    void BiosGuardClass::decodeBgsl(UINT8* fv, INT64 length) {
        if ((length % 8) != 0) {
        ...
        for (INT32 idx = 0; idx < NumOfOp; ++idx) {
            BgslOperation OpLine = *(BgslOperation*)(fv + idx * 8);
