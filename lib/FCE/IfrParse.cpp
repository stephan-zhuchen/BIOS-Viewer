...
//    Count = vsprintf(mStringBuffer + mCount, FormatString, Marker);
    Count = vsprintf_s(mStringBuffer + mCount, sizeof(mStringBuffer) - mCount, FormatString, Marker);
  ...
  )
{
  ...
  while (!IsNull (StorageListHead, Link)) {
    ...
    if (!CompareGuid (&StorageNode->Guid, &CurNode->Guid)   \
      && (CurNode->Name != nullptr)                            \
      && (StorageNode->Name != nullptr)                        \
  ...
  )
{
  ...
  assert ((UniPackge != nullptr) && (CurUniPackge != nullptr));

  HiiStringHeader  = nullptr;
  Offset           = 1;
  Count            = 0;
  Block            = nullptr;
  ThisBlock        = nullptr;
  ...
  while( Block->BlockType != EFI_HII_SIBT_END ) {
    ...
    case EFI_HII_SIBT_STRING_UCS2:
      ...
      *VarDefaultName = (CHAR16*)malloc ((FceStrLen ((CHAR16 *) ((EFI_HII_SIBT_STRING_UCS2_BLOCK *)ThisBlock)->StringText) + 1) * sizeof (CHAR16));
      if (*VarDefaultName == nullptr) {
  ...
  )
{
  ...
  //
  if (VarDefaultNameId == 0) {
    *VarDefaultName = (CHAR16*)L"";
    return EFI_SUCCESS;
  }

  if (CachedStringList != nullptr) {
  ...
  case UQI:
    strcpy_s (LangStr, "uqi");
    break;

  case EN_US:
    strcpy_s (LangStr, "en-US");
    break;

  case ENG:
    strcpy_s (LangStr, "eng");
    break;
...
Done:
  ...
  if (CachedStringList != nullptr) {
    if (CachedStringList->CachedIdNum >= CachedStringList->MaxIdNum) {
      TempBuffer = (STRING_INFO*)calloc (sizeof (STRING_INFO), CachedStringList->MaxIdNum + STRING_NUMBER);
    ...
    if ((FormSet->FormSetOrder == CurNode->FormSetOrder) \
      ...
      if (CurNode->Type == EFI_IFR_VARSTORE_EFI_OP) {
        ...
        if (!CurNode->NewEfiVarstore) {
          if (CurNode->Buffer == nullptr) {
            CurNode->Buffer = (UINT8*)malloc (Question->StorageWidth);
          }
          if (CurNode->Name == nullptr) {
        ...
        //
         if ((CurNode->Name == nullptr) || (FceStrLen (CurNode->Name) == 0)) {
        ...
        if (Question->VariableName == nullptr) {
  ...
  )
{
  ...
  ASSERT (Statement->Storage != nullptr);
  ...
  if (Statement->Storage->Type == EFI_HII_VARSTORE_NAME_VALUE) {
    ...
    ASSERT (Statement->VariableName != nullptr);
    //
    // Insert to Name/Value varstore list
    //
    NameValueNode = (NAME_VALUE_NODE*)AllocateZeroPool (sizeof (NAME_VALUE_NODE));
    ASSERT (NameValueNode != nullptr);
    NameValueNode->Signature = NAME_VALUE_NODE_SIGNATURE;
    NameValueNode->Name = (CHAR16*)FceAllocateCopyPool (FceStrSize (Statement->VariableName), Statement->VariableName);
    ASSERT (NameValueNode->Name != nullptr);
    NameValueNode->Value = (CHAR16*)AllocateZeroPool (0x10);
    ASSERT (NameValueNode->Value != nullptr);
    NameValueNode->EditValue = (CHAR16*)AllocateZeroPool (0x10);
    ASSERT (NameValueNode->EditValue != nullptr);
  ...
  )
{
  ...
  Expression = (FORM_EXPRESSION*)AllocateZeroPool (sizeof (FORM_EXPRESSION));
  ...
  )
{
  ...
  Storage = (FORMSET_STORAGE*)AllocateZeroPool (sizeof (FORMSET_STORAGE));
  ...
  )
{
  ...
  while (!IsListEmpty (&Expression->OpCodeListHead)) {
    ...
    if (OpCode->ValueList != nullptr) {
      FreePool (OpCode->ValueList);
    }

    if (OpCode->ValueName != nullptr) {
      FreePool (OpCode->ValueName);
    }

    if (OpCode->MapExpressionList.ForwardLink != nullptr) {
  ...
  )
{
  ...
  while (!IsListEmpty (&Storage->NameValueListHead)) {
    ...
    if (NameValueNode->Name != nullptr) {
      FreePool (NameValueNode->Name);
    }
    if (NameValueNode->Value != nullptr) {
      FreePool (NameValueNode->Value);
    }
    if (NameValueNode->EditValue != nullptr) {
  ...
  )
{
  ...
  //
  if (StorageEntryListHead->ForwardLink != nullptr) {
  ...
  )
{
  ...
  while (!IsListEmpty (&Statement->DefaultListHead)) {
    ...
    if (Default->Value.Buffer != nullptr) {
  ...
  if (Statement->VariableName != nullptr) {
    FreePool (Statement->VariableName);
  }
  if (Statement->BufferValue != nullptr) {
  ...
  )
{
  ...
  //
  if (FormSet->DefaultStoreListHead.ForwardLink != nullptr) {
  ...
  // Free Forms
  //
  if (FormSet->FormListHead.ForwardLink != nullptr) {
  ...
  if (FormSet->StatementBuffer != nullptr) {
    FreePool (FormSet->StatementBuffer);
  }
  if (FormSet->ExpressionBuffer != nullptr) {
    FreePool (FormSet->ExpressionBuffer);
  }
  if (FormSet->EnUsStringList.StringInfoList != nullptr) {
  ...
  if (FormSet->UqiStringList.StringInfoList != nullptr) {
  ...
  )
{
  ...
  // Parse Fromset one by one
  //
  if (FormSetEntryListHead->ForwardLink != nullptr) {
  ...
  )
{
  ...
  CurrentDefault           = nullptr;
  CurrentOption            = nullptr;
  OptionSuppressExpression = nullptr;
  FormSuppressExpression   = nullptr;
  MapScopeDepth            = 0;
  Link                     = nullptr;
  VarStorage               = nullptr;
  MapExpressionList        = nullptr;
  TempVarstoreId           = 0;
  ConstantFlag             = TRUE;
  ...
  mStatementIndex = 0;
  FormSet->StatementBuffer = (FORM_BROWSER_STATEMENT*)AllocateZeroPool (NumberOfStatement * sizeof (FORM_BROWSER_STATEMENT));
  if (FormSet->StatementBuffer == nullptr) {
    return EFI_OUT_OF_RESOURCES;
  }

  mExpressionOpCodeIndex = 0;
  FormSet->ExpressionBuffer = (EXPRESSION_OPCODE*)AllocateZeroPool (NumberOfExpression * sizeof (EXPRESSION_OPCODE));
  if (FormSet->ExpressionBuffer == nullptr) {
    return EFI_OUT_OF_RESOURCES;
  }
  ...
  while (OpCodeOffset < FormSet->IfrBinaryLength) {
    ...
    if (IsExpressionOpCode (Operand)) {
      ...
      case EFI_IFR_EQ_ID_VAL_LIST_OP:
        ...
        ExpressionOpCode->ValueList = (UINT16*)FceAllocateCopyPool (ExpressionOpCode->ListLength * sizeof (UINT16), &((EFI_IFR_EQ_ID_VAL_LIST *) OpCodeData)->ValueList);
        break;
      ...
      case EFI_IFR_THIS_OP:
        ASSERT (CurrentStatement != nullptr);
      ...
      case EFI_IFR_SET_OP:
        ...
        if (TempVarstoreId != 0) {
          if (FormSet->StorageListHead->ForwardLink != nullptr) {
            Link = GetFirstNode (FormSet->StorageListHead);
            while (!IsNull (FormSet->StorageListHead, Link)) {
          ...
          if (ExpressionOpCode->VarStorage == nullptr) {
            //
        ...
        if ((ExpressionOpCode->VarStorage != nullptr) &&
          ...
          if (ExpressionOpCode->ValueName == nullptr) {
            //
      ...
      if (Operand == EFI_IFR_MAP_OP) {
        ...
        if (mInScopeDisable && (CurrentForm == nullptr)) {
          //
    ...
    case EFI_IFR_FORM_MAP_OP:
      ...
      //
      CurrentForm = (FORM_BROWSER_FORM*)AllocateZeroPool (sizeof (FORM_BROWSER_FORM));
      ASSERT (CurrentForm != nullptr);
    ...
    case EFI_IFR_VARSTORE_OP:
      ...
      Storage->Buffer = (UINT8*)AllocateZeroPool (Storage->Size);

      AsciiString = (CHAR8 *) ((EFI_IFR_VARSTORE *) OpCodeData)->Name;
      Storage->Name = (CHAR16*)AllocateZeroPool ((strlen (AsciiString) + 1) * 2);
      ASSERT (Storage->Name != nullptr);
      ...
      //
      if ((TempStorage = NotSameVariableInVarList (FormSet->StorageListHead, Storage)) != nullptr) {
    ...
    case EFI_IFR_DEFAULTSTORE_OP:
      HaveInserted = FALSE;
      DefaultStore = (FORMSET_DEFAULTSTORE*)AllocateZeroPool (sizeof (FORMSET_DEFAULTSTORE));
      ASSERT (DefaultStore != nullptr);
    ...
    case EFI_IFR_ORDERED_LIST_OP:
      ...
      CurrentStatement->BufferValue   = nullptr;
      if (Scope != 0) {
        SuppressForOption = TRUE;
      }
      //
    ...
    case EFI_IFR_STRING_OP:
      ...
      CurrentStatement->BufferValue = (UINT8*)AllocateZeroPool (CurrentStatement->StorageWidth + sizeof (CHAR16));
      //
      // Get the UQI information
      //
      PrintQuestion(FormSet, CurrentForm, CurrentStatement, FALSE);
      ...
      Status = GetGuidNameByVariableId (FormSet, CurrentStatement, FormSet->StorageListHead);
      //
      // Remove the question of which isn't stored by EfiVarStore or VarStore
      //
      if (EFI_ERROR (Status)) {
        RemoveEntryList (&CurrentStatement->Link);
        DestroyStatement (FormSet, CurrentStatement);
    ...
    case EFI_IFR_DATE_OP:
      ...
      if ((CurrentStatement->Flags & EFI_QF_DATE_STORAGE) == QF_DATE_STORAGE_NORMAL) {
        ...
        //
        CurrentStatement->Storage = nullptr;
    ...
    case EFI_IFR_DEFAULT_OP:
      ...
      //
      CurrentDefault = (QUESTION_DEFAULT*)AllocateZeroPool (sizeof (QUESTION_DEFAULT));
      ...
      if (CurrentDefault->Value.Type == EFI_IFR_TYPE_BUFFER) {
        ...
        CurrentDefault->Value.Buffer = (UINT8*)FceAllocateCopyPool(CurrentDefault->Value.BufferLen, &((EFI_IFR_DEFAULT *)OpCodeData)->Value);
        ASSERT(CurrentDefault->Value.Buffer != nullptr);
      } else {
    ...
    case EFI_IFR_ONE_OF_OPTION_OP:
      ...
      if (CurrentStatement->Operand == EFI_IFR_ORDERED_LIST_OP &&
        ...
        CurrentDefault->Value.Buffer = (UINT8*)FceAllocateCopyPool(CurrentDefault->Value.BufferLen, &((EFI_IFR_ONE_OF_OPTION *)OpCodeData)->Value);
        ASSERT(CurrentDefault->Value.Buffer != nullptr);
      ...
      // It creates a selection for use in current Question.
      //
      CurrentOption = (QUESTION_OPTION*)AllocateZeroPool (sizeof (QUESTION_OPTION));
      ASSERT (CurrentOption != nullptr);
      ...
      //
      if ((CurrentStatement->Operand == EFI_IFR_ORDERED_LIST_OP) && (CurrentStatement->BufferValue == nullptr)) {
        Width = 1;
        ...
        CurrentStatement->BufferValue = (UINT8*)AllocateZeroPool (CurrentStatement->StorageWidth);
    ...
    case EFI_IFR_DISABLE_IF_OP:
      ...
      //
      CurrentExpression = (FORM_EXPRESSION*)AllocateZeroPool (sizeof (FORM_EXPRESSION));
      ASSERT (CurrentExpression != nullptr);
    ...
    case EFI_IFR_END_OP:
      ...
      case EFI_IFR_WARNING_IF_OP:
        //
        // Ignore the end of EFI_IFR_NO_SUBMIT_IF and EFI_IFR_INCONSISTENT_IF
        //
        break;
...
ReGetDefault:
  ...
  //
  // EFI_IFR_DEFAULT has the highest priority
  //
  if (!IsListEmpty (&Question->DefaultListHead)) {
    ...
    while (!IsNull (&Question->DefaultListHead, Link)) {
      ...
      if (Default->DefaultId == DefaultId) {
        if (Default->ValueExpression != nullptr) {
          //
  ...
  // The default id in DefaultStoreList is in ascending order to make sure to choose the smallest default id every time.
  //
  while (!IsNull(&FormSet->DefaultStoreListHead, DefaultLink)) {
  ...
  case EFI_IFR_ONE_OF_OP:
    ...
    //
    if (ValueToOption (FormSet, Question, HiiValue) == nullptr) {
      Link = GetFirstNode (&Question->OptionListHead);
  ...
  )
{
  ...
  if (SettingScope == FormLevel) {
    ...
    while (!IsNull (FormSetEntryListHead, FormLink)) {
      ...
      ExtractDefault (FormSet, nullptr, DefaultId, PlatformId, FormSetLevel);
  ...
  )
{
  ...
  if (&(FormSet->EnUsStringList) == nullptr && VarDefaultName != nullptr && FormSet->FormSetTitle != 0) {
    free (VarDefaultName);
    VarDefaultName = nullptr;
  }
}
  ...
  )
{
  ...
  while (!IsNull (&Question->OptionListHead, Link)) {
    ...
    LogUnicodeString (VarDefaultName);
    StringPrint("\n");
    if (&(FormSet->EnUsStringList) == nullptr && VarDefaultName != nullptr && Option->Text != 0) {
      free (VarDefaultName);
      VarDefaultName = nullptr;
    }
    Link = GetNextNode (&Question->OptionListHead, Link);
  ...
  )
{
  ...
  if (&(FormSet->EnUsStringList) == nullptr && VarDefaultName != nullptr && Form->FormTitle != 0) {
    free (VarDefaultName);
    VarDefaultName  = nullptr;
  }
  ...
  )
{
  ...
  if (PrintOrNot) {
    ...
    assert (!EFI_ERROR (Status));
    if ((VarDefaultName != nullptr) && (FceStrCmp (VarDefaultName, (CHAR16*)L"") != 0)) {
  ...
  if (&(FormSet->EnUsStringList) == nullptr &&VarDefaultName != nullptr && Question->Prompt != 0) {
    free (VarDefaultName);
    VarDefaultName = nullptr;
  }
  ...
  if (PrintOrNot && Question->Storage) {
    ...
    LogUnicodeString (VarDefaultName);
    StringPrint("\n");
    if (&(FormSet->EnUsStringList) == nullptr &&VarDefaultName != nullptr && Question->Help != 0) {
      free (VarDefaultName);
      VarDefaultName = nullptr;
    }
  }
  ...
  )
{
  ...
  if (mMultiPlatformParam.MultiPlatformOrNot) {
    ...
    if (mMultiPlatformParam.Uqi.Data != nullptr) {
      StringPrint("\n\n//FCEKEY PLATFORM_UQI:");
  ...
  while (!IsNull (FormSetEntryListHead, FormSetLink)) {
    ...
    if (CheckFormSetOrFormNull (FormSet, nullptr, TRUE)) {
    ...
    while (!IsNull (&FormSet->FormListHead, FormLink)) {
      ...
      if (CheckFormSetOrFormNull (nullptr, Form, FALSE)) {
      ...
      while (!IsNull (&Form->StatementListHead, QuestionLink)) {
        ...
        if ((Question->Operand == EFI_IFR_ONE_OF_OP)
          ...
          //
          ConstantFlag    = TRUE;
          if (!Skip && (Question->DisableExpression != nullptr)) {
