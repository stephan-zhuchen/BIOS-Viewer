  ...
  )
{
  ...
  NewStack = (EFI_HII_VALUE*)AllocatePool (Size * sizeof (EFI_HII_VALUE));
  if (NewStack == NULL) {
  ...
  )
{
  ...
  case EFI_IFR_TYPE_NUM_SIZE_64:
    ...
    case EFI_IFR_STRING_SIGNED_DEC:
      PrintFormat = (CHAR16*)L"%ld";
      break;

    case EFI_IFR_STRING_LOWERCASE_HEX:
      PrintFormat = (CHAR16*)L"%lx";
      break;
  ...
  )
{
  ...
  if (Value.Type == EFI_IFR_TYPE_STRING) {
    ...
    StringPtr = StrStr (String, (CHAR16*)L"0X");
    if (StringPtr != NULL) {
      //
      // Hex string
      //
  ...
  )
{
  ...
  StringPtr= (CHAR16*)AllocatePool (FceStrSize (String[1]) + Size);
  ...
  while (!IsNull (&Expression->OpCodeListHead, Link)) {
    ...
    case EFI_IFR_TO_BOOLEAN_OP:
        ...
        if (Value->Type <= EFI_IFR_TYPE_DATE) {
          ...
          if (FceStrCmp (StrPtr, (CHAR16*)L"TRUE") == 0){
            Value->Value.b = TRUE;
          } else if (FceStrCmp (StrPtr, (CHAR16*)L"FALSE") == 0) {
            Value->Value.b = FALSE;
          } else {
            Status = EFI_INVALID_PARAMETER;
            FreePool (StrPtr);
